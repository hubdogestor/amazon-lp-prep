const case_3 = {
  ...
  isGoodCase: true,
  pt: {
    s: `Em 2022, ao assumir a área de Risco de Pagamentos da Unimed, uma analista me trouxe um alerta que parecia trivial: um pequeno aumento em reembolsos de psicologia. Mas ao mergulhar nos dados, minha intuição apontou para algo maior. Eu notei um padrão estranho: milhares de reembolsos de R$150, sempre para os mesmos 12 psicólogos, solicitados por 112 clientes diferentes. O sistema de fraude existente, que analisava transações isoladamente, não via nada de errado, mas a projeção indicava uma perda anual de R$4.8 milhões, minando a confiança dos nossos 1.6 milhão de beneficiários.`,
    ...
    r: `Minha investigação estancou uma perda de R$4.8 milhões anuais. A precisão do nosso sistema de detecção aumentou em 31%, enquanto os falsos positivos para clientes legítimos caíram 42%, melhorando a experiência do usuário. O tempo de revisão manual de alertas caiu de 25 para 11 minutos. Mais importante, o mapa de correlação que eu criei se tornou um mecanismo permanente, que foi posteriormente replicado no Sicredi para reduzir fraudes no PIX em 45%.`,
